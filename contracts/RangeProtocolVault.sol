 ...
 * It has mint and burn functions for the users to provide liquidity in tokenX and tokenY and has
    ...
    RangeProtocolVaultStorage
{
    ...
    function initialize(
        ...
        (
            ...
            address _priceOracleX,
            address _priceOracleY,
            address _otherFeeRecipient,
            address _otherFeeClaimer
        ...
        state.priceOracleX = _priceOracleX;
        state.priceOracleY = _priceOracleY;
        state.otherFeeClaimer = _otherFeeClaimer;
        VaultLib.setOtherFeeRecipient(state, _otherFeeRecipient);

        // Managing fee is 0% performance is 10% and other fee at 30% at the time vault initialization.
    ...
    function swap(
        ...
        return VaultLib.swap(state, zeroForOne, swapAmount, pointLimit, minAmountOut);
    }
    ...
    function collectOtherFee() external override {
        if (msg.sender != state.otherFeeClaimer) revert VaultErrors.OnlyOtherFeeClaimer();
     ...
     * @param newOtherFee other fee percentage out of 10_000
     */
    function updateFees(
