...
interface IRangeProtocolVault is IERC20Upgradeable, IiZiSwapCallback, IiZiSwapMintCallback {
    ...
    function priceOracleX() external view returns (address);

    function priceOracleY() external view returns (address);
    ...
    function otherBalanceX() external view returns (uint256);

    function otherBalanceY() external view returns (uint256);
